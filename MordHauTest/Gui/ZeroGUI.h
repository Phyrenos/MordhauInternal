...
wchar_t* s2wc(const char* c)
{
	...
	mbstowcs_s(nullptr, wc, cSize, c, cSize - 1);  // Using the safer version
...
namespace ZeroGUI
{
	...
	UCanvas* canvas;
	int active_section = -1; // -1 means no section is active by default
	...
	void TextLeft(const char* name, FVector2D pos, FLinearColor color, bool outline, FVector2D Scale)
	{
		int length = strlen(name) + 1;
		canvas->K2_DrawText(Variables::font, FString{ s2wc(name) }, pos, Scale, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, false, true, true, Colors::Text_Outline);
	}
	void TextLeft(const char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		int length = strlen(name) + 1;
		canvas->K2_DrawText(Variables::font, FString{ s2wc(name) }, pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, false, true, true, Colors::Text_Outline);
	}
	void TextCenter(const char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		int length = strlen(name) + 1;
		canvas->K2_DrawText(Variables::font, FString{ s2wc(name) }, pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, true, true, true, Colors::Text_Outline);
	}
	...
	bool Window(const char* name, FVector2D* pos, FVector2D size, bool isOpen)
	{
		...
		// Red Border
		Draw_Line(FVector2D{ pos->X, pos->Y }, FVector2D{ pos->X + size.X, pos->Y }, 2, Colors::Window_Header); // Top
		Draw_Line(FVector2D{ pos->X, pos->Y + size.Y }, FVector2D{ pos->X + size.X, pos->Y + size.Y }, 2, Colors::Window_Header); // Bottom
		Draw_Line(FVector2D{ pos->X, pos->Y }, FVector2D{ pos->X, pos->Y + size.Y }, 2, Colors::Window_Header); // Left
		Draw_Line(FVector2D{ pos->X + size.X, pos->Y }, FVector2D{ pos->X + size.X, pos->Y + size.Y }, 2, Colors::Window_Header); // Right
	...
	bool SectionHeader(const std::list<std::string>& items, int* selection)
	{
		bool isActive = false;
		elements_count++;

		FVector2D size = FVector2D{ 50, 20 };
		...
		{
			bool isHovered = MouseInZone(FVector2D{ pos.X, pos.Y }, size);
			bool isSelected = (*selection == current_id);

			if (isHovered || isSelected)
			{
				
				drawFilledRect(FVector2D{ pos.X, pos.Y }, size.X, size.Y,
					isSelected ? Colors::Button_Active : Colors::Button_Hovered);
				hover_element = true;
			}

			FLinearColor textColor = Colors::Text;
			FVector2D textPos = FVector2D{ pos.X + size.X / 2, pos.Y + size.Y / 2 };
			TextCenter(item.c_str(), textPos, textColor, false);
	...
	FVector2D TextSize(FString text, FVector2D size) {
		return canvas->K2_TextSize(Variables::font, text, size);
	}
	...
	void SliderFloat(const char* name, float* value, float min, float max, const char* format = "%.1f%%")
	{
		elements_count++;
